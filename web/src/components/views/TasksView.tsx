...
export default function TasksView({ project, onOpen }: Props) {
  ...
  return (
    <div>
      ...
      {docs.length === 0 ? (
        <div className="text-center text-muted-foreground p-8 bg-white border border-border rounded-xl">暂无任务</div>
      ) : (
        docs.map((d) => {
          const done = (d.tasks || []).filter((t) => t.done).length
          const pct = (d.tasks || []).length ? Math.round((done / (d.tasks || []).length) * 100) : 0
          return (
            <div key={d.path} className="bg-white border border-border rounded-xl p-4 mb-4">
              <div className="flex items-center gap-2 mb-2">
                <TypeTag type={d.type} />
                <button onClick={() => onOpen(d)} className="font-semibold hover:underline cursor-pointer">{d.title}</button>
                <span className="text-xs text-muted-foreground">{d.date || ''}</span>
                <div className="h-1.5 flex-1 bg-gray-100 rounded-full overflow-hidden ml-auto">
              ...
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-0.5">
                {d.tasks.map((t, i) => (
                  <li key={i} className={`flex items-start gap-2 text-[13px] py-0.5 ${t.done ? 'text-gray-400 line-through' : ''}`}>
