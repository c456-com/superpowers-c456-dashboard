...
- **五视图深挖**：总览 · Roadmap 时间线 · 功能设计图谱 · 开发计划甘特图 · 任务清单（checkbox 进度）
...
- **类型**：路径含 `specs` 或标题含「设计/方案」→ spec；`plans` 或含「实现计划」→ plan；含 `roadmap/路线图` → roadmap；含 `sprint/冲刺` → sprint；其余 → doc
- **元数据**：标题后 `> 状态：` / `> **键：** 值` 引用块（支持两种键风格）
